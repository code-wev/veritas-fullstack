...
const USERS = [
  {
    email: 'admin.demo@cgveritas.test',
    password: 'DemoAdmin2026!',
    name: 'Demo Admin',
    role: 'admin'
  },
  {
    email: 'partner.demo@cgveritas.test',
  ...
  {
    email: 'client.demo@cgveritas.test',
...
async function main() {
  ...
  for (const u of USERS) {
    ...
    // Special handling for client user assignment
    if (u.role === 'client_user') {
      ...
      if (clients && clients.length > 0) {
        ...
        console.log(`Assigned client user to client: ${targetClient.name}`);
      } else {
  ...
  console.log('All roles seed completed successfully.');
}
