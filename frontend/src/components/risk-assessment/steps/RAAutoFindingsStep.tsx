...
const SEVERITY_FROM_RATING: Record<string, string> = {
  Low: 'low',
  Medium: 'medium',
  High: 'high',
  Critical: 'high',
...
export function RAAutoFindingsStep({ reviewId, engagementId }: Props) {
  ...
  const load = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('risk_assessment_control_results')
        .select('id, section_name, control_area, control_objective, comments, observation_best_practice, response, risk_rating')
  ...
  const pushToFindings = async () => {
    ...
    try {
      const autoRows = deficiencies.map((d) => ({
        ...
        severity: SEVERITY_FROM_RATING[d.risk_rating ?? ''] ?? 'medium',
        original_severity: SEVERITY_FROM_RATING[d.risk_rating ?? ''] ?? 'medium',
        status: 'draft',
              ...
              <div key={d.id} className="border rounded-md p-3 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex gap-1">
                    ...
                    {d.risk_rating && <Badge variant="outline" className="text-xs">{d.risk_rating}</Badge>}
                  </div>
                </div>
