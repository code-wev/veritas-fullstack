...
export function RAChecklistStep({ reviewId }: { reviewId: string }) {
    ...
    try {
      ...
      if (existing && existing.length > 0) {
        ...
        const { data: templates, error: tplErr } = await supabase
          .from('risk_assessment_question_templates')
          .select('*')
          .eq('is_active', true)
          .order('sort_order');
                    ...
                    <Card key={item.id} className={item.deficiency_flag ? 'border-destructive/50' : ''}>
                      <CardContent className="p-4 space-y-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <Input
                            placeholder="e.g. Risk Assessment v2.1, Methodology doc, Section 3.2..."
